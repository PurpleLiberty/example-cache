...
public class OpenshiftIT {
	private static final String NAME_SERVICE_APP = "openliberty-cache-cute-name";
	private static final String GREETING_SERVICE_APP = "openliberty-cache-greeting";
	...
	public void firstRequestShouldBeSlow() {
		...
		assertThat("Server responded too fast, expected at least 2000 ms, got response in " + time, time,
	...
	public void secondRequestShouldBeFast() {
		...
		assertThat("Server didn't respond fast enough. Expecting response in at most 1000 ms, got in " + time, time,
	...
	private void assertCached(boolean cached) {
		given().baseUri(greetingServiceUrl).when().get("/api/cached").then().body("cached", is(cached));
	}

	private String getGreeting() {
		String greeting = given().baseUri(greetingServiceUrl).when().get("/api/greeting").then().statusCode(200)
				.body("message", startsWith("Hello")).extract().jsonPath().get("message");
		return greeting;
	}

	private void clearCache() {
		await().atMost(1, TimeUnit.MINUTES).untilAsserted(() -> {
			given().baseUri(greetingServiceUrl).when().delete("/api/cached").then().statusCode(204);
		});
